import queue from '{origin}{basePath}/async/queue'
...
const q = queue(async ({ imports, testFn, $li, $status }) => {
  ...
  try {
    const modules = Array.isArray(imports) ? await Promise.all(imports.map(n => {
      return import(`{origin}{basePath}/${n}`)
    })) : []
    const module = typeof imports === 'string' ? await import(`{origin}{basePath}/${imports}`) : undefined
