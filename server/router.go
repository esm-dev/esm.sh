...
func esmRouter(esmStorage storage.Storage, logger *log.Logger) rex.Handle {
	...
	return func(ctx *rex.Context) any {
		...
		if query.Has("meta") {
			...
			if len(buildMeta.Integrity) == 0 || !strings.HasPrefix(buildMeta.Integrity, "sha-384-") {
				...
				integrity = "sha-384-" + base64.StdEncoding.EncodeToString(sha.Sum(nil))
			...
			metaJson["integrity"] = "sha384-" + integrity[8:]
			ctx.SetHeader("Content-Type", ctJSON)
			if isExactVersion {
				ctx.SetHeader("Cache-Control", ccImmutable)
			} else {
