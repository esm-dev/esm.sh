...
const template = (widgets: DevtoolsWidget[]) =>
  html`
  <div id="dev-tools">
    ...
    <ul>
      ...
      <li>
        <button class="icon">
          <svg class="logo" width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" xmlns="http://www.w3.org/2000/svg">
  ...
  <style>
    ...
    #dev-tools > ul > li:has(> button:focus) > .widget,
    #dev-tools > ul > li:hover > .widget {
      visibility: visible;
    }
    #dev-tools > ul > li > button.icon {
      background-color: rgba(150, 150, 150, 0.15);
      opacity: 0.75;
    }
    #dev-tools > ul > li:hover > button.icon {
      opacity: 1;
...
function renderWidget(w: DevtoolsWidget): string {
  return html`
    <li>
      <div class="widget"><${w.component}></${w.component}></div>
      <button class="icon">${w.icon}</button>
    </li>
  `;
}
