...
// CachDir returns the cache directory for the given app name.
func CachDir(appName string) (string, error) {
	if Windows {
		appDataDir := os.Getenv("LOCALAPPDATA")
		if appDataDir != "" {
...
// GetDenoPath returns the path to the Deno executable.
func DenoPath(appDataDir string) (denoPath string, err error) {
	if appDataDir == "" {
