...
Deno.test("drop invalid `?external`", async () => {
  const res1 = await fetchModule("react-dom@18.3.1?target=es2022&external=foo,bar,react");
  const code1 = await res1.text();
  assertStringIncludes(code1, '"/react-dom@18.3.1/X-ZXJlYWN0/es2022/react-dom.mjs"');

  const res2 = await fetchModule("react-dom@18.3.1?target=es2022&external=foo,bar,preact");
  const code2 = await res2.text();
  assertStringIncludes(code2, '"/react-dom@18.3.1/es2022/react-dom.mjs"');

  const res3 = await fetchModule("react-dom@18.3.1?external=react-dom");
  const code3 = await res3.text();
  assertStringIncludes(code3, '"/react-dom@18.3.1/denonext/react-dom.mjs"');
});

Deno.test("types with `?external`", async () => {
  const res = await fetchModule("swr@1.3.0/X-ZXJlYWN0/dist/use-swr.d.ts");
...
Deno.test("external nodejs internal modules", async () => {
  const res = await fetchModule("cheerio@0.22.0/es2022/cheerio.mjs");
  assertEquals(res.status, 200);
  assertStringIncludes(await res.text(), ` from "/node/buffer.js"`);

  const res2 = await fetchModule("cheerio@0.22.0?target=es2022&external=node:buffer");
  res2.body?.cancel();
  ...
  const res4 = await fetchModule("*cheerio@0.22.0?target=es2022");
  res4.body?.cancel();
