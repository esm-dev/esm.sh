...
function fromFileUrl(url) {
  ...
  let path4 = decodeURIComponent(url.pathname.replace(/\//g, '\\').replace(/%(?![0-9A-Fa-f]{2})/g, '%25')).replace(
    /^\\*([A-Za-z]:)(\\|$)/,
    '$1\\'
  )
  if (url.hostname != '') {
...
function globToRegExp(
  glob,
  { extended = true, globstar: globstarOption = true, os = osType, caseInsensitive = false } = {}
) {
  if (glob == '') {
    return /(?!)/
  }
...
function pathToFileURL(filepath) {
  ...
  if (
    (filePathLast === CHAR_FORWARD_SLASH || (isWindows && filePathLast === CHAR_BACKWARD_SLASH)) &&
...
export { _url as URL, url_default as default, fileURLToPath, pathToFileURL }
